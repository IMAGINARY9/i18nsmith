...
export class VueWriter implements I18nWriter {
  ...
  private findCandidateOffset(
    ...
    content: string,
    options: { includeQuotes?: boolean; preferQuoted?: boolean } = {}
    ...
    const tryExactAt = (column: number) => {
  ...
  const targetIndex = lineStart + Math.max(candidate.position.column - 1, 0);
    ...
    const targetIndex = lineStart + Math.max(candidate.position.column - 1, 0);
    let searchIndex = 0;
    let bestNeighbor: { start: number; end: number; distance: number } | null = null;
  ...
  private expandToSurroundingQuotes(offset: { start: number; end: number }, content: string): { start: number; end: number } {
