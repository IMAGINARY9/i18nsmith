...
import { buildLocaleDiffs, KeyGenerator, KeyValidator, LocaleStore, Scanner, generateValueFromKey, } from '@i18nsmith/core';
import { DEFAULT_TRANSLATABLE_ATTRIBUTES } from '@i18nsmith/core';
import { detectExistingTranslationImport, ensureClientDirective, ensureUseTranslationBindingWithOptions, ensureUseTranslationImport, findNearestFunctionScope, hasUseTranslationBinding, isComponentLikeFunction, } from './react-adapter.js';
...
export class Transformer {
    ...
    async processReactFile(relativePath, plans, progressState, emitProgress, skippedFiles, changedFiles, write, generateDiffs, runOptions, fileImportCache) {
        let sourceFile;
        let originalContent = '';
        try {
            ...
            for (const plan of plans) {
                ...
                const internalCandidate = { ...plan, raw: rawCandidate };
                try {
                    ...
                    const componentScope = internalCandidate.raw.node.getFirstAncestor((ancestor) => {
                        if (Node.isFunctionDeclaration(ancestor) ||
                            ...
                            return isComponentLikeFunction(ancestor);
                        }
                        return false;
                    });
                    ...
                    if (write || generateDiffs) {
                        didMutate = this.writers.find(w => w instanceof ReactWriter).applyCandidate(internalCandidate);
                        if (didMutate) {
                            const relativeModulePath = this.getRelativeModulePath(adapterForFile.module, plan.filePath);
                    ...
                    if (didMutate && !wasExisting) {
                        const sourceValue = (await this.findLegacyLocaleValue(this.sourceLocale, plan.text)) ??
                            plan.text ??
            ...
            const reason = `Failed to process file: ${error.message}`;
            for (const plan of plans) {
                progressState.processed += 1;
                plan.status = 'skipped';
                plan.reason = reason;
    ...
    async processVueFile(filePath, plans, progressState, emitProgress, skippedFiles, changedFiles, write, runOptions, sourceDiffs) {
        ...
        try {
            ...
            const { content: newContent, didMutate } = await this.writers.find(w => w instanceof VueWriter).transform(filePath, originalContent, plans);
            ...
            const reason = `Failed to process Vue file: ${error.message}`;
            for (const plan of plans) {
                progressState.processed += 1;
                plan.status = 'skipped';
                plan.reason = reason;
    ...
    async run(runOptions = {}) {
        ...
        const originalLocaleData = new Map();
        if (generateDiffs) {
        ...
        const transformableByFile = new Map();
        ...
        const diffs = generateDiffs
            ...
            : [];
        const candidateStats = preparedCandidates.reduce((acc, candidate) => {
            acc.total += 1;
        ...
        const skippedReasons = preparedCandidates.reduce((acc, candidate) => {
    ...
    async generateDiffs(originalData) {
        const projectedData = new Map();
        const storedLocales = await this.localeStore.getStoredLocales();
        const allLocales = new Set([this.sourceLocale, ...this.targetLocales, ...storedLocales]);
        for (const locale of allLocales) {
            projectedData.set(locale, await this.localeStore.get(locale));
        }
        return buildLocaleDiffs(originalData, projectedData, (locale) => this.localeStore.getFilePath(locale), this.workspaceRoot);
    }
    checkDependencies() {
     ...
     */
    getUnsafeJsxExpressionReason(node) {
    ...
    containsConditionalOrLogical(node) {
    ...
    applyCandidate(candidate) {
        ...
        if (candidate.kind === 'jsx-expression') {
            ...
            if (expression) {
                if (this.normalizeTranslationSnippet(expression.getText()) ===
            ...
            if (this.normalizeTranslationSnippet(node.getText()) ===
    ...
    buildLegacyKeyCandidates(rawKey) {
    ...
    normalizeTranslationSnippet(text) {
        return text
            .replace(/\s+/g, '')
            .replace(/`/g, '\'')
            .replace(/"/g, '\'')
 ...
 */
Transformer.ADAPTER_DEPENDENCIES = {
